...
zend_object_value create_detecter(zend_class_entry* type TSRMLS_DC)
{
    ...
    zend_hash_copy(detecter_obj->std.properties, &type->default_properties,
        static_cast<copy_ctor_func_t>(zval_add_ref), static_cast<void*>(&tmp), sizeof(zval*));
...
protected:
    static void prepare(wdt::Detecter* detecter, const wdt::PrepareOpts& opts, wdt::PrepareRes& res);

    static void locate(wdt::Detecter* detecter, const wdt::LocateOpts& opts, wdt::LocateRes& res);

    static void detect(wdt::Detecter* detecter, const wdt::DetectOpts& opts, wdt::DetectRes& res);
